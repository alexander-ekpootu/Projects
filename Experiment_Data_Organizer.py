...
# encoding: utf-8
# coder:    Alexander Ekpo-Otu, Charlie Glass(UI)
# version:  2.0
...
fra.SetIcon(wx.Icon(wx.Bitmap('images/app_icon.ico')))
...
src_file_ent.SetHint('Enter source file...')
src_file_sizer.Add(src_file_ent, 1, flag=wx.ALIGN_CENTER)
src_file_sizer.AddSpacer(3)


def src_file_sel_com(event):
	dialog = wx.FileDialog(None, "Select source file", wildcard="Excel Files (.xlsx)|*.xlsx",
						   style=wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
	dialog.ShowModal()
	path = dialog.GetPath()
	if path:
		src_file_ent.SetValue(path)
...
def del_st_btn_com(event):
	inp_file = src_file_ent.GetValue()
	out_dir = out_fold_ent.GetValue()
	mess = ''

	if not os.path.exists(inp_file):
		mess = 'Make sure source file is specified and it exists.'
	...
	if mess:
		...
		def pro_exec():
			...
			for r_i, row in enumerate(inp_data):
				if row[0] == 'Subject:':
					...
					steps = 0
					for nxt_rows in inp_data[r_i:]:
						if nxt_rows[0] == 'M:': break
						steps += 1

					for eve in inp_data[r_i + 1:r_i + steps]:
						col_data_l[sub].extend(eve)
			...
			out_sht = out_wb.active

			for row in fin_out_data:
				out_sht.append(row)

			out_wb.save(out_dir + '\\H Output.xlsx')
			out_wb.close()
			...
			out_wb.save(out_dir + '\\D Output.xlsx')
			out_wb.close()
			...
			out_wb.save(out_dir + '\\L Output.xlsx')
			out_wb.close()
		...
		threading.Thread(target=pro_exec, daemon=True).start()
